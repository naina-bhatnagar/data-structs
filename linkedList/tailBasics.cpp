...
public:
    ...
    void pop_front(){
        ...
        Node* temp = head;
        head = head->next;
        if(head==NULL) tail=NULL; // if the head becomes NULL, append tail as NULL as well
        delete temp;
    }
    ...
    void pop_back(){
        ...
        temp->next = NULL; // cut the connection
        delete tail; // delete the old tail {not the variable but the memory stored internally in a heap}
        tail = temp; // update tail to the new value
    }
